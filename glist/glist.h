...
class GLIST final: public CoreMaintenance {
 public:
  ...
  void EdgeInsert(std::vector<std::pair<int, int>> & edges,
                   std::vector<std::vector<int>>& graph,
                   std::vector<int>& core,const int K,
                   std::vector<bool>& evicted_vi);

  void EdgeRemove(std::vector<std::pair<int, int>> &edges,
                   std::vector<std::vector<int>>& graph,
                   std::vector<int>& core,const int K,
                   std::vector<bool>& evicted_vr);

  void ComputeFollower(const int u,
                   std::vector<std::vector<int>>& graph,
                   std::vector<int>& core, std::vector<int>& follower,int K);

  void ComputeFollower_bruteforce(const int u,const int v,
            std::vector<std::vector<int>>& graph,
            std::vector<int>& core, std::vector<int>& follower,int K);
  void AnchoredKorder(const int u,
                   std::vector<std::vector<int>>& graph,
                   std::vector<int>& core,int K);
  void AnchoredKorder1(const int u,
            std::vector<std::vector<int>>& graph,
  ...
  void Keep_customed(const std::vector<std::vector<int>>& graph,
            const int v, const int K,
            const std::vector<int>& core,
            int& list_t, std::vector<int>& swap);
