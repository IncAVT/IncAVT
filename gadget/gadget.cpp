...
namespace gadget {
    ...
    std::vector<std::pair<int, int>> ReadEdgesS(const char* const path,
        ...
        while (true) {
            ...
            if (feof(file)) break;
            // for debug purpose
            DEBUG_PRINT("ReadEdgesS:edge: %d %d\n",v1,v2);
            ASSERT(0 <= v1 && v1 < *n && 0 <= v2 && v2 < *n && v1 != v2);
            if (v1 > v2) std::swap(v1, v2);
            const auto e = std::make_pair(v1, v2);
            //DEBUG_PRINT("ReadEdgesS:edge: %d %d\n",e.first,e.second);
            ...
            // insert the edge
            edges.push_back(e);
            ASSERT(static_cast<int>(debug.size()) == static_cast<int>(edges.size()));
            //DEBUG_PRINT("debug size:%d\n",static_cast<int>(debug.size()));
            //DEBUG_PRINT("edge size:%d\n",static_cast<int>(edges.size()));
        }
        DEBUG_PRINT("final debug size:%d\n",static_cast<int>(debug.size()));
        DEBUG_PRINT("final edge size:%d\n",static_cast<int>(edges.size()));
        ASSERT(static_cast<int>(debug.size()) == *m);
        ASSERT(static_cast<int>(edges.size()) == *m);
        DEBUG_PRINT("claimed edge size:%d\n",*m);
